...
SCRIPT_TOTAL=2
# Vulnerable variants
SCRIPT_NUM=1 SCRIPT_TOTAL=$SCRIPT_TOTAL python3 run-slither.py \
  --root reentr-assert-vuln \
  --out $RESULTS_DIR/vuln-slither \
  --timeout "$TIMEOUT" $RERUN_TIMEOUTS
# Safe variants
SCRIPT_NUM=2 SCRIPT_TOTAL=$SCRIPT_TOTAL python3 run-slither.py \
  --root reentr-assert-safe \
  --out $RESULTS_DIR/safe-slither \
  --timeout "$TIMEOUT" $RERUN_TIMEOUTS
