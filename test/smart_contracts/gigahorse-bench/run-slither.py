#!/usr/bin/env python3
"""Run slither on a benchmark suite."""
...
def main():
    parser = argparse.ArgumentParser(description="Run slither on a benchmark suite.")
    parser.add_argument("--root", required=True,
                        help="Root directory containing benchmark sources")
    parser.add_argument("--out", required=True,
                        help="Output directory for results and timings CSV")
    parser.add_argument("--detect", metavar="D1,D2,...",
                        default="reentrancy-eth,reentrancy-no-eth,reentrancy-balance",
                        help="Comma-separated list of slither detectors to enable "
                             "(default: reentrancy-eth,reentrancy-no-eth,reentrancy-balance)")
    ...
    print(f"\n{'='*60}")
    print(f"  *** SLITHER *** — running on {ROOT}")
    print(f"{'='*60}\n")
    ...
    for i, sol in enumerate(sols, 1):
        ...
        # Build a temporary directory alongside src/ containing all source files
        # plus YulTracerLib.sol from the include dir, so slither sees everything
        # as local files (avoiding path-with-spaces issues in solc remappings).
        ...
        try:
            ...
            issues = [d["check"] for d in detectors]
        except Exception:
            issues = []
