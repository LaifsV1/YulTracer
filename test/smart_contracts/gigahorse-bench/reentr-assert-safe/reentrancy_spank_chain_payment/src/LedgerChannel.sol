...
abstract contract Token {
     ...
     function totalSupply() view virtual returns (uint256 supply);
     is replaced with:
 ...
 library ECTools {
     ...
     function parseInt16Char(string memory _char) public pure returns (uint256) {
         bytes memory bresult = bytes(_char);
         // bool decimals = false;
         if ((bresult[0] >= bytes1(uint8(48))) && (bresult[0] <= bytes1(uint8(57)))) {
             return uint256(uint8(bresult[0])) - 48;
         } else if ((bresult[0] >= bytes1(uint8(65))) && (bresult[0] <= bytes1(uint8(70)))) {
             return uint256(uint8(bresult[0])) - 55;
         } else if ((bresult[0] >= bytes1(uint8(97))) && (bresult[0] <= bytes1(uint8(102)))) {
             return uint256(uint8(bresult[0])) - 87;
         } else {
             revert();
         }
     }

     // @dev Converts a uint256 to a bytes32
     // @thanks https://ethereum.stackexchange.com/questions/4170/how-to-convert-a-uint256-to-bytes-in-solidity
     function uintToBytes32(uint256 _uint) public pure returns (bytes memory b) {
         b = new bytes(32);
     ...
     // @dev Converts a uint256 in a string
     ...
     function substring(string memory _str, uint256 _startIndex, uint256 _endIndex) public pure returns (string memory) {
 ...
 contract HumanStandardToken is StandardToken {
     ...
     */
     ...
     function approveAndCall(address _spender, uint256 _value, bytes memory _extraData) public returns (bool success) {
         ...
         //VK: require(_spender.call(bytes4(bytes32(keccak256("receiveApproval(address,uint256,address,bytes)"))), msg.sender, _value, this, _extraData));
 ...
 contract LedgerChannel {
         ...
         payable
     {
         ...
         require(_partyI != address(0x0), "No partyI address provided to LC creation");
     ...
     function LCOpenTimeout(bytes32 _lcID) public {
         ...
         if(curr_channel.initialDeposit[1] != 0) {
             // <yes> <report> REENTRANCY
             require(curr_channel.token.transfer(curr_channel.partyAddresses[0], curr_channel.erc20Balances[0]),"CreateChannel: token transfer failure");
         }

         emit DidLCClose(_lcID, 0, curr_channel.ethBalances[0], curr_channel.erc20Balances[0], 0, 0);

         Yult.Assert(address(this).balance >= eth_prev - amount);
         assert(address(this).balance >= eth_prev - amount);
     }
     ...
     //Struct Getters
     function getChannel(bytes32 id) public view returns (address[2] memory, 
         uint256[4] memory, 
         uint256[4] memory, 
         uint256[2] memory, 
         uint256, 
     ...
     function getVirtualChannel(bytes32 id) public view returns (bool, 
         bool, 
         uint256, 
         address, 
         uint256, 
