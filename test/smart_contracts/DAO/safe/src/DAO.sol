...
contract ManagedAccount is ManagedAccountInterface{
    ...
    function payOut(address _recipient, uint _amount) public override payable returns (bool) {
        require(!(msg.sender != owner || msg.value > 0 || (payOwnerOnly && _recipient != owner)));
...
abstract contract TokenCreation is TokenCreationInterface, Token {
    ...
    function createTokenProxy(address _tokenHolder) public override payable returns (bool success) {
        require ((block.timestamp < closingTime && msg.value > 0
...
contract DAO is DAOInterface, TokenCreation {
  ...
  /// FIX: simple non‐reentrant guard
  ...
  modifier nonReentrant() {
    ...
    constructor(
        ...
        require(!(address(rewardAccount) == address(0)));
        require(!(address(DAOrewardAccount) == address(0)));
    ...
    function newProposal(
        ...
        require(!(block.timestamp + _debatingPeriod < block.timestamp)); // prevents overflow
    ...
    function executeProposal(
        ...
        // If we are over deadline and waiting period, require proposal is closed
        ...
        require (!(block.timestamp < p.votingDeadline  // has the voting deadline arrived?
        ...
        if (quorum >= minQuorum(p.amount)) {
            require(!(!p.creator.send(p.proposalDeposit)));
    ...
    function splitDAO(
        ...
        if (address(p.splitData[0].newDAO) == address(0)) {
            ...
            require (!(address(p.splitData[0].newDAO) == address(0)));
            // should never happen
            require(!(address(this).balance < sumOfProposalDeposits));
        ...
        uint256 beforeBalance = address(this).balance;           // CUSTOM VARIABLE
        
        // Move ether and assign new Tokens
        uint fundsToBeMoved =
            (balances[msg.sender] * p.splitData[0].splitBalance) /
            p.splitData[0].totalSupply;
        require(!(p.splitData[0].newDAO.createTokenProxy{value : fundsToBeMoved}(msg.sender) == false));
        ...
        require(!(rewardToken[address(this)] < rewardTokenToBeMoved));
        rewardToken[address(this)] -= rewardTokenToBeMoved;

        DAOpaidOut[address(p.splitData[0].newDAO)] += paidOutToBeMoved;
        require(!(DAOpaidOut[address(this)] < paidOutToBeMoved));
        ...
        uint reward = (balanceOf(msg.sender) * rewardAccount.accumulatedInput()) / totalSupply - paidOut[msg.sender];  // FIX: Reward has to be computed before state update
        totalSupply -= balances[msg.sender]; // FIX: state update moved before withdrawRewardsFor
        balances[msg.sender] = 0;            // FIX:
        paidOut[msg.sender] = 0;             // FIX:
        withdrawRewardFor2(msg.sender, reward); // be nice, and get his rewards

        Yult.Assert(beforeBalance - address(this).balance == fundsToBeMoved); // CUSTOM ASSERTION
        assert(beforeBalance - address(this).balance == fundsToBeMoved); // CUSTOM ASSERTION
        return true;
    }
    ...
    function retrieveDAOReward(bool _toMembers) external override returns (bool _success) {
        ...
        require(!((rewardToken[msg.sender] * DAOrewardAccount.accumulatedInput()) /
        ...
        uint reward =
            ...
            totalRewardToken - DAOpaidOut[msg.sender];
        if(_toMembers) {
            require(DAOrewardAccount.payOut(address(dao.rewardAccount()), reward));
            }
        else {
            require(DAOrewardAccount.payOut(address(dao), reward));
        }
    ...
    function getMyReward() public override returns (bool _success) {
     ...
     // FIX: this function is called after state update; rewards are calculated previously
    function withdrawRewardFor2(address _account, uint reward) internal returns (bool _success) {
        require(!((balanceOf(_account) * rewardAccount.accumulatedInput()) / totalSupply < paidOut[_account]));

        require(rewardAccount.payOut(_account, reward));
    ...
    function transferFromWithoutReward(
        ...
        require(!(!withdrawRewardFor(_from)));
    ...
    function transferPaidOut(
        ...
        require(! (transferPaidOut_ > paidOut[_from]));
    ...
    function changeProposalDeposit(uint _proposalDeposit) external override {
       require(!(msg.sender != address(this) || _proposalDeposit > (actualBalance() + rewardToken[address(this)])
            / maxDepositDivisor)) ;
        proposalDeposit = _proposalDeposit;
    }
