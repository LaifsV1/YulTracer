...
sed -i'.bak' '/function[[:space:]]\{1,\}fun___yult__revealUint_[0-9]\{1,\}(/! s/fun___yult__revealUint_[0-9]\{1,\}(/REVEAL_UINT(/g' "$1" && rm "$1.bak"
echo "⤷ Processing __yult__revealUint"

sed -i'.bak' '/function[[:space:]]\{1,\}fun___yult__revealAddress_[0-9]\{1,\}(/! s/fun___yult__revealAddress_[0-9]\{1,\}(/REVEAL_ADDR(/g' "$1" && rm "$1.bak"
