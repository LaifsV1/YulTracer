...
(* Comments added by the compiler to show the signature that corresponds to a selector hash *)
let sig_comment =
...
rule token = parse
  ...
  | comments           { (token lexbuf) }                (* match comments before everything else *)
  | "object"           { OBJECT }
  | "code"             { CODE }
  | "data"             { DATA "data" }
  ...
  | eof                { EOF }
  | _                  { raise (Yul_semantics.Error.LexE (get_lex_start_end_p lexbuf,
