 ...
 *)
module Sto = struct (* TODO: perhaps we can use this generally for Tries *)
  ...
  let get (key : U256.t) (map : t) :(evm_val) =
    ...
    | None ->
       failwith
         (Printf.sprintf "tried to access memory at uninitialised address <%s>"
            (Z.to_string key))
    | Some v -> v
...
type t =
  {
    main_trie        : U256.t BytesMap.t; 
    storage_tries    : Sto.t BytesMap.t; 
    snapshots        : (U256.t BytesMap.t * Sto.t BytesMap.t) list;
