  ...
  | U256_val dividend :: U256_val divisor :: [] ->
     ...
     let quotient =
       if Z.equal divisor Z.zero then
          ...
          *)
         let is_neg = (Z.sign dividend) = -1 || (Z.sign divisor) = -1 in
         let abs_result = (U256._floordiv_ (Z.abs dividend) (Z.abs divisor)) in
         if is_neg then Z.neg abs_result else abs_result
     in
     {evm with stack = [U256_val quotient]}
  (* SYMBOLIC CASE. note: this assumes only U256 and S256, no other constructors. *)
...
let symbolic_mod_triop evm x y z sop =
     ...
     in
     let sigma , new_var = sigma_fresh_bv256_var evm.symbolic_env result_prop in
  ...
  | U256_val byte_num :: U256_val value :: [] ->     
     ...
     let result =
       if Z.gt byte_num (U256.of_int 31) then
         ...
         (* NOTE: the following line is necessary to convert a small negative number into a large 2's complement number of size 256bits *)
  ...
  | U256_val byte_num :: S256_val value :: [] ->
     ...
     if Z.gt byte_num (U256.of_int 31) then
       ...
       (* The following line implements the same operation as our concrete case above.
          ...
          - `mk_extract` expects a `low` and `high` bit range. We need to ensure that
       ...
       let signed_sigma , signed_var = sigma_fresh_bv256_var evm.symbolic_env signed_prop in

       {evm with stack = [S256_val signed_var]; symbolic_env = signed_sigma}
