(** Ethereum Virtual Machine (EVM) Control Flow Instructions for Yul
    @author Yu-Yang Lin
    created: 15-Nov-2023

    Implementations of the EVM control flow instructions based on EVM Specs. 
 *)

(** {@python[from ethereum.base_types import U256, Uint]} *)
(* open Base_types *)

(** {@python[from ...vm.gas import GAS_BASE, GAS_HIGH, GAS_JUMPDEST, GAS_MID, charge_gas]} *)
open Gas
...
let stop (evm : EVM.t) :(EVM.t) = {evm with running = false}
...
let pc (evm : EVM.t) :(EVM.t) = do_pushval evm evm.pc _GAS_BASE
...
let gas_left (evm : EVM.t) :(EVM.t) = do_pushval evm evm.gas_left _GAS_BASE
