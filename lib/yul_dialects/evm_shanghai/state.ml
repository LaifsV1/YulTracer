(** Module for Storage Model
    @author Yu-Yang Lin
    created: 01-Mar-2024

    This file implements the module for our model of storage.
    This module diverges from the original specifications and serves only in
    analogy to the storage utilities in the specifications.

    The State struct is actually conceptually analogous to the Blockchain
    itself. It holds the state that would be on the Blockchain.
    
    Note: Storage is Word-indexed. 
 *)
...
(* MODULE FOR STORAGE *)
(* ------------------ *)
(** module for symbolic storage map.
    this model is based on the assumption that Keccak do not produce conflicts and
    keccak addresses are only ever computed on concrete inputs.
 *)
module Sto = ZToEvmMap

(* ------------------------------------- *)
(* IMPLEMENTATION OF ACTUAL STATE MODULE *)
(* ------------------------------------- *)
...
type t =
  {
    main_trie        : Account.t BytesMap.t; 
    storage_tries    : Sto.t BytesMap.t; 
    (* snapshots        : (Account.t BytesMap.t * Sto.t BytesMap.t) list; *)
    (* created_accounts : unit BytesMap.t (* map to unit is a set, may want to change this *) *)
  }

let to_string ({main_trie;storage_tries} : t) :(string) =
  let module Formatter = MapFormatter(BytesMap) in
  let main    = Formatter.map_to_string Bytes.to_hex_string Account.to_string main_trie in
  let storage = Formatter.map_to_string Bytes.to_hex_string Sto.to_string storage_tries in
  Printf.sprintf "State{main_trie = %s; storage_tries = %s}" main storage

(** analogous to [default_factory] initialisation functions *)
let empty :(t) =
  {
    main_trie        = BytesMap.empty; 
    storage_tries    = BytesMap.empty; 
    (* snapshots        = []; *)
    (* created_accounts = BytesMap.empty *)
  }
 ...
 *)
(* this isn't necessary, we are functional *)
...
def get_account_optional(state: State, address: Address) -> Optional[Account]:
    ...
    return account
 *)
let get_account_optional (state : t) (address : address) :(Account.t option) =
  BytesMap.find_opt address state.main_trie
 ...
 *)
let get_account (state : t) (address : address) :(Account.t) =
  let account = get_account_optional state address in
  match account with
 ...
 *)
let set_account (state : t) (address : address) (account : Account.t option) :t =
  let main_trie = BytesMap.update address (fun _ -> account) state.main_trie in 
  { state with main_trie }
...
def get_storage(state: State, address: Address, key: Bytes) -> U256:
    ...
    NOTE: we are using `U256.t` for `key` instead of `Bytes`
          we assume that we will only ever need to access Storage with `U256.t`
 *)
let get_storage (state : t) (address : Bytes.t) (key : U256.t) :(evm_val) =
  match BytesMap.find_opt address state.storage_tries with
  | None -> U256_val U256.zero
  | Some trie -> Sto.get key trie (* no need to assert because of types *)
...
def set_storage(
    ...
    if trie._data == {}:
        del state._storage_tries[address]
 *)
let set_storage (state : t) (address : Bytes.t) (key : U256.t) (value : evm_val) :(t) =
  let trie =
    match BytesMap.find_opt address state.storage_tries with
    | None -> Sto.empty 
    | Some trie -> trie
  in
  let new_storage = Sto.set key value trie in
  let new_storage_tries = BytesMap.add address new_storage state.storage_tries in
  {state with storage_tries = new_storage_tries}
 ...
 *)
let account_exists (state : t) (address : address) :(bool) =
  match get_account_optional state address with
  | None -> false
  | _ -> true
 ...
 *)
let account_has_code_or_nonce (state : t) (address : address) :(bool) =
  let account = get_account state address in
  account.nonce <> U256.zero || account.code <> []
 ...
 *)
let is_account_empty (state : t) (address : address) :(bool) =
  let account = get_account state address in
  (
 ...
 *)
let account_exists_and_is_empty (state : t) (address : address) :(bool) =
  let account = get_account_optional state address in
  match account with
  | None -> false
  | Some account ->
     (Z.equal account.nonce Z.zero)
     && (account.code = [])
     && (account.balance = Z.zero)
 ...
 *)
let is_account_alive (state : t) (address : address) :(bool) =
  let account = get_account_optional state address in
  match account with
  | None -> false
  | Some account ->
     not(
         U256.is_zero account.nonce 
         && account.code = []
         && U256.is_zero account.balance
        )
 ...
 *)
let modify_state (state : t) (address : address) (f : Account.t -> Account.t) :t =
  set_account state address (Some (f (get_account state address)))
...
let move_ether
    ...
    :(t)
  =
  (* NOTE: we are functional, not imperative *)
  let reduce_sender_balance (sender: Account.t) :Account.t =
    ensure (sender.balance >= amount , AssertionError)
      ("[move_ether]: sender " ^ (Bytes.to_hex_string sender_address)
       ^ " has insufficient balance (" ^ (U256.to_string sender.balance)
       ^ ") to transfer " ^ U256.to_string amount);
  ...
  let state = modify_state state sender_address reduce_sender_balance in
  let state = modify_state state recipient_address increase_recipient_balance in
  state
 ...
 *)
let increment_nonce (state : t) (address : address) :(t) =
    let increase_nonce (sender: Account.t) =
      {sender with nonce = U256.succ sender.nonce}
    in
    modify_state state address increase_nonce
 ...
 *)
let set_code (state : t) (address : address) (code : Bytes.t) :(t) =
    let write_code (sender: Account.t) =
      {sender with code = code}
    in
    modify_state state address write_code
