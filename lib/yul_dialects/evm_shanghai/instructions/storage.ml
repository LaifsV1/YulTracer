...
let sload (evm : EVM.t) :(EVM.t) = 
  ...
  (* Symbolic Storage will prune the trace, but won't cause tool to fail anymore. *)
  | S256_val _ :: _ as s ->
     Printf.printf "\x1b[1;33mWARNING!\x1b[0m Trace pruned: symbolic pointer <%s> encountered in [sload]; symbolic addressing unsupported.\n"
       (String.concat "::" (List.map EvmVal.to_string s)) ;
     {evm with running = false}
  | s ->
     let stack_str = String.concat "::" (List.map EvmVal.to_string s) in
     failwith(Printf.sprintf "sload: malformed stack <%s>" stack_str)
...
let sstore (evm : EVM.t) :(EVM.t) = 
  ...
  | U256_val key :: new_value :: [] ->
     ...
     (* get_storage(evm.env.state, evm.message.current_target, key) *)
       ...
       "[sstore]: cannot modify storage in a static context";
  ...
  | s ->
     let stack_str = String.concat "::" (List.map EvmVal.to_string s) in
     failwith(Printf.sprintf "sstore: malformed stack <%s>" stack_str)
