...
module K = struct
  ...
  let to_string (k : t) :(string) =
    Printf.sprintf "Keccak{%s}"
      begin
        let module Formatter = MapFormatter(BytesMap) in
        Formatter.map_to_string Bytes.to_string U256.to_string k
      end
          ...
          begin
            ...
            if Bytes.is_concrete v then                    (* if the input `v` is a concrete U256 *)
              ...
              BytesMap.fold                                (* for every... *)
                begin
                  fun (w_i : Bytes.t) z_i acc ->                       (* let `z_i` = `K(w_i)` *)
                  ...
                  if List.length v = List.length w_i then
                    begin
                  ...
                  let sigma_n =
                    BytesMap.fold                          (* ...for every... *)
                      (fun w_i _ (acc_sigma) ->            (* add `(v =/= w_i)` to sigma *)
                        if List.length w_i = List.length v then
