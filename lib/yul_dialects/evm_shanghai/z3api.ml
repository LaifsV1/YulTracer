...
let string_of_option f = function Some x -> f x | None -> "None"
let string_of_pair f g (a,b) = Printf.sprintf "(%s , %s)" (f a) (g b)
...
let bv8_numeral_z3 s   :(Expr.expr) = BitVector.mk_numeral ctx s 8
...
let rec string_of_prop = function
  (* constants *)
  | ConstBV256 z -> Z.to_string z
  | ConstBV8   z -> Z.to_string z
  | ConstTrue    -> "true"
  | ConstFalse   -> "false"
  (* variables *)
  | VarBV256 w -> string_of_symbolic_var w
  | VarBV8   w -> string_of_symbolic_var w
  | VarBool  w -> string_of_symbolic_var w
  (* bit-vector operations -- done this way for extensibility *)
  | BVunop  (op,p)     -> Printf.sprintf "(%s %s)" (string_of_unary_op op) (string_of_prop p)
  | BVbinop (op,p1,p2) -> Printf.sprintf "(%s %s %s)"
                            (string_of_binary_op op) (string_of_prop p1) (string_of_prop p2)
  (* bit-vector operations that take integer input *)
  | BVExtract (i,j,p) -> Printf.sprintf "(extract %d %d %s)" i j (string_of_prop p)
  | BVSignExt (i,p)   -> Printf.sprintf "(sign_extract %d %s)" i (string_of_prop p)
  (* bit-vector operations for concatenation *)
  | BVConcat ps -> Printf.sprintf "(concat [%s])" (string_of_sequence "," string_of_prop ps)
  (* boolean operations -- top-level because extensibility is not needed *)
  | NotProp p  -> Printf.sprintf "(not %s)" (string_of_prop p)
  | OrProp  ps -> Printf.sprintf "(or [%s])" (string_of_sequence "," string_of_prop ps)
  | AndProp ps -> Printf.sprintf "(and [%s])" (string_of_sequence "," string_of_prop ps)
  | XorProp (p1,p2) -> Printf.sprintf "(xor %s %s)" (string_of_prop p1) (string_of_prop p2)
  | EqProp  (p1,p2) -> Printf.sprintf "(eq %s %s)" (string_of_prop p1) (string_of_prop p2)
  | ImpliesProp (p1,p2) -> Printf.sprintf "(implies %s %s)" (string_of_prop p1) (string_of_prop p2)
  | IteProp  (p1,p2,p3) ->  Printf.sprintf "(ite %s %s %s)"
                              (string_of_prop p1) (string_of_prop p2) (string_of_prop p3)
...
let rec z3_of_prop (prop : prop) :(Expr.expr) =
  ...
  | ConstBV256 z -> bv256_numeral_z3 (Z.format "%d" z)
  | ConstBV8   z -> bv8_numeral_z3 (Z.format "%d" z)
  | ConstTrue -> true_z3
  | ConstFalse -> false_z3
...
let string_of_prop_via_z3 (prop : prop) :(string) = string_of_z3 (z3_of_prop prop) (* todo: adhoc? *)
...
let fresh_svar () =
  ...
  {str=default_sname;id=new_id}

let fresh_named_svar str =
  let new_id = get_fresh_symbolic_id () in
  {str=str;id=new_id}
...
(* to string functions *)
let string_of_sigma_via_z3 sigma = string_of_sequence " and \n" string_of_prop_via_z3 sigma
let string_of_sigma sigma = string_of_sequence " and \n" string_of_prop sigma

(* functions to traverse and print with sort *)
let rec traverse_print_z3_expr (e : Expr.expr) : unit =
  let s = Expr.get_sort e in
  Printf.printf "Expr: %s | Sort: %s\n" (Expr.to_string e) (Sort.to_string s);
  List.iter traverse_print_z3_expr (Expr.get_args e)

let traverse_print_sigma (sigma : sigma) : unit =
  let es = List.map z3_of_prop sigma in
  List.iter traverse_print_z3_expr es

(* check function for sigma *)
let check_sigma_sat sigma =
  try
    check (List.map z3_of_prop sigma)
  with
  | Z3.Error msg ->
     print_endline "============== CHECK SAT SIGMA ERROR ==============";
     Printf.printf "PRINTING SIGMA:\n%s\n" (string_of_sigma sigma);
     failwith msg
