(** Numeric & Array Types
    ...
    NOTE: file deleted from specs in commit: a150464eceb9362d685877292ebb552e8fe473f2 (Sep 30, 2024).
    This was replaced with the ethereum-types repository.
    Refer to files from commit: 3fe6514f2d9d234e760d11af883a47c1263eff51.
    {:https://github.com/ethereum/execution-specs/blob/3fe6514f2d9d234e760d11af883a47c1263eff51/src/ethereum/base_types.py}
...
module Fixed_uint (MAX_VALUE : Zval) = struct
  ...
  let to_hex_string (z : t) :string = format "%x" z

  let to_ascii_string (z : Z.t) : string =
    (* Extract bytes, least significant first *)
    let rec extract_bytes z acc =
      if Z.equal z Z.zero then acc
      else
        let byte = Z.extract z 0 8 |> Z.to_int in
        let z' = Z.shift_right z 8 in
        extract_bytes z' (Char.chr byte :: acc)
    in
    (* Remove trailing '\x00' from the list *)
    let rec strip_trailing_zeroes = function
      | [] -> []
      | '\000' :: tl ->
         let stripped = strip_trailing_zeroes tl in
         if stripped = [] then [] else '\000' :: stripped
      | c :: tl -> c :: strip_trailing_zeroes tl
    in
    let chars = extract_bytes z [] |> strip_trailing_zeroes in
    String.of_seq (List.to_seq chars)
  
  (** make random number *)
  let random () =
    (* Random.init 9814072356; *)
    (* This is STDLIB Bytes module, not the same as the Bytes below *)
  ...
  (** [to_be_bytes] extracts `byte_length` 8-bit bytes from the number and
      ...
      NOTE: this PREPENDS 0s by shifting. i.e. 32-bytes with left padding.
      e.g.
  ...
  let _add_ : t -> t -> t =
    ...
    then failwith (Printf.sprintf "_add_: result out of range for <%s> , <%s>"
  ...
  let wrapping_add : t -> t -> t =
    fun left right ->
    (* we check both left and right *)
    (* TODO: the left is not checked in the specs *)
    if out_of_range right
    then failwith (Printf.sprintf "wrapping_add: right arg out of range for <%s> , <%s>"
                     (to_string left) (to_string right))
    else
  ...
  let _sub_ : t -> t -> t =
    ...
    then failwith (Printf.sprintf "_sub_: result out of range for <%s> , <%s>"
                     (to_string left) (to_string right))
    else left - right
  ...
  {@python[
    def __mul__(self: T, right: int) -> T:
        ...
        # NOTE: result > self.MAX_VALUE was added at some point, and then deleted
        if right < 0 or result > self.MAX_VALUE: 
            raise ValueError()
  ...
  let _mul_ : t -> t -> t =
    ...
    let result = left * right in
    (* we check both left and right *)
    if Z.lt right Z.zero (* || Z.gt result max_value *)
    then failwith (Printf.sprintf "_mul_: result out of range for <%s> , <%s>"
                     (to_string left) (to_string right))
    else result
  ...
  let _mod_ : t -> t -> t =
    fun left right ->
   (* if out_of_range left then assert(false) else *)
    if out_of_range right then assert(false)
    else (left mod right)
...
(** Custom function to get a 2's complement 256-bit big-int [zint].
    ...
    assumes 2's complement with infinite leading 1's and returns a
    nonnegative number. e.g. a two's complement 1111 becomes positive 1111. *)
let u8_extract (x : zint) :(zint) = Z.extract x 0 8
...
module EvmVal = struct
  ...
  let format str = function
    ...
    | S256_val s -> string_of_symbolic_var s
  let signed_extract = function
    | U256_val z -> u256_signed_extract z
    | _ -> failwith "cannot extract symbolic values"
end
...
let string_of_byte = function U8 z -> Z.format "%x" z | S8 svar -> string_of_symbolic_var svar
let b s = U8 (Z.of_string s)
...
let ascii_of_byte (b : byte) : string =
  match b with
  | U8 z ->
      let i = Z.to_int z in
      if i >= 32 && i <= 126 then
        (* printable ASCII → single-char string *)
        String.make 1 (Char.chr i)
      else
        (* non-printable → open box "□" (U+25A1) *)
        "\xE2\x96\xA1"
  | S8 _ ->
      (* symbolic → black box "■" (U+25A0) *)
      "\xE2\x96\xA0"

(** Module for our own implementation of symbolic Bytes.
    These are implemented as lists of `byte` defined above, which are either U8 or S8.
    Comparison and conversion between `evm_val` and `Bytes.t` is done here.
 *)
module Bytes = struct
  type t = byte list
  ...
  let to_string bytes =
    let string_list =
      List.map string_of_byte bytes
    in
    Printf.sprintf "[%s]" (String.concat "," string_list)
  
  (** Convert a Bytes.t into an ASCII/placeholder string *)
  let to_ascii_string (bytes : t) : string =
    (* Map each byte to its 1- or 3-byte string, then glue them together *)
    bytes
    |> List.map ascii_of_byte
    |> String.concat ""
  
  let take_32 bytes =
    let take_pad_drop n pad xs =
      let rec aux acc i xs =
        if i = 0 then (List.rev acc, xs)
        else match xs with
             | x :: xs' -> aux (x :: acc) (Stdlib.(-) i 1) xs'
             | [] -> aux (pad :: acc) (Stdlib.(-) i 1) []
      in
      aux [] n xs
    in
    take_pad_drop 32 (U8 Z.zero) bytes
  ...
  (** function to extract a list of concrete bytes iff the input contains only U8 *)
  let to_concrete_bytes (bytes : t) :(zint list) =
    List.map (function U8 x -> x | _ -> failwith "[to_concrete_bytes]: S8") bytes

  (** function to extract bytes of a list of U8 bytes and then convert the extract list into U256.t *)
  let to_U256 (bytes : t) :(zint) =
    U256.from_be_bytes (to_concrete_bytes bytes)

  (** function to get a Bytes.t from a list of zint numbers *)
  let of_concrete_bytes (z : zint list) :(t) = List.map (function x -> U8 x) z

  (** function to get a Bytes.t from a Z.t *)
  let of_U256 (z : zint) :(t) = of_concrete_bytes (U256.to_be_bytes z)
  ...
  let evm_val_from_be_bytes (bytes : t) (sigma : sigma) :(evm_val * sigma) =
    (* Printf.printf "bytes: %s; size: %d\n" (to_string bytes) (List.length bytes); *)
    if List.length bytes <> 32 then
      ...
      if is_symbolic bytes then
        ...
        U256_val (U256.from_be_bytes (to_concrete_bytes bytes)) , sigma
  ...
  (** function to get the hex string of a byte array *)
  let to_hex_string bytes = "0x"^(bytes |> to_U256 |> U256.to_hex_string)

  (** function to get the hex string of a byte array *)
  let to_dec_string bytes = (bytes |> to_U256 |> U256.to_string)
...
(** formatter for any Map.S *)
module MapFormatter (M : Map.S) = struct
  (** function for converting maps to strings *)
  let map_to_string 
        (key_to_string : M.key -> string) 
        (value_to_string : 'v -> string) 
        (map : 'v M.t) : string =
    
    let elements = M.fold 
                     (fun key value acc ->
                       Printf.sprintf "\n(%s --> %s)" (key_to_string key) (value_to_string value) :: acc)
                     map [] 
    in
    String.concat "; " (List.rev elements)  (* Reverse for correct order *)

  (** function for printing map contents directly *)
  let iter_print (key_to_string : M.key -> string) (value_to_string : 'v -> string) (map : 'v M.t) =
    M.iter (fun key value ->
        Printf.printf "%s --> %s\n" (key_to_string key) (value_to_string value)) map
end
