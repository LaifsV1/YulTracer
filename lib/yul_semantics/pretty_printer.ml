...
module YulPrettyPrinter (D : Dialect) = struct
  ...
  and print_expression expression' =
    ...
    | Str s ->
       ...
       pr s;
       pr "\""
    | Tuple vs -> (* NOTE: tuples are special values, not source code, so we decide how to print them. *)
       pr "<";
       print_list print_value print_comma vs;
       pr ">"

  (** functions to print Yul Objects *)
  let rec print_yul_object {name;code;members;_} =
    (* object %s { code %s %s } *)
    pr "object ";
    pr "\"";
    pr name;
    ...
    force_newline ();
    print_list print_yul_member force_newline members;
    close_box ();
    force_newline ();
    pr "}";
    print_cut (); 
