...
def load_contracts(json_file):
    try:
        ...
        print(f"{ERROR}unable to read JSON: {e}", file=sys.stderr)
        sys.exit(1)
...
def main():
    ...
    for paths, obj, num in extracted:
        ...
        if len(matches) == 1:
            ...
            print(f"{ERROR}no match for '{name_key}'. Tried:")
            for c in candidates:
                print(f"  -> {c}")

        else:
            print(f"{WARNING}duplicate matches for '{name_key}':")
            for m in matches:
                print(f"  -> {m}")
    ...
    print(f"{DONE}Wrote {len(output_list)} contracts to '{out_path}'")
