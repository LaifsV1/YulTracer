...
echo "${STEP}Extracting matching names from '$file1' and '$file2' with pattern '$pattern'..."

names1=($(grep -E "$pattern" "$file1" | grep -o '"name": *"[^"]*"' | sed 's/"name": *"\([^"]*\)"/\1/'))
names2=($(grep -E "$pattern" "$file2" | grep -o '"name": *"[^"]*"' | sed 's/"name": *"\([^"]*\)"/\1/'))

if [ ${#names1[@]} -eq 0 ]; then
  echo "${ERROR}No matching names found in '$file1'. Exiting."
  exit 0
fi

if [ ${#names2[@]} -eq 0 ]; then
  echo "${ERROR}No matching names found in '$file2'. Exiting."
  exit 0
fi

if [ ${#names1[@]} -ne ${#names2[@]} ]; then
  echo "${WARN}Number of matching names differ between files."
  echo "${WARN}$file1 has ${#names1[@]} matches, $file2 has ${#names2[@]} matches."
  echo "${WARN}Proceeding with replacement for minimum count of matches: $(( ${#names1[@]} < ${#names2[@]} ? ${#names1[@]} : ${#names2[@]} ))"
fi
